...
def scalesize(x_np_pts, scale = 255, offset = False):
    if offset == False:
    ...
    for array in cloud:
    	maximo = np.max(array)
    	if maximo < scale-1:
    		cloud2.append(array + (((scale-1)/2)-(maximo/2)) + offset)
    	else:
    		cloud2.append(array + offset)
...
def pcdtovoxel(cloud,scale = 255, offset = False):
	if offset == False:
		offset = int(scale*.1 + 1) 
	voxel_cube = np.zeros((scale + 2*offset, scale + 2*offset, scale + 2*offset))
...
def getimagefromvoxel(voxel_cube,scale = 255, view = 0, offset = False):
	if offset == False:
		offset = int(scale*.1 + 1) 
	image = np.zeros((scale + 2*offset, scale + 2*offset))
	if view <= 2:
		for i in range(scale+2*offset):
			for j in range(scale+2*offset):
				for k in range(scale+2*offset):
					if view == 0:
		...
		for i in range(scale+2*offset):
			for j in range(scale+2*offset):
				cn = offset
				for k in range(scale+2*offset-1,-1,-1):
					cn += 1
					if view == 3:
						if voxel_cube[i,j,k] == 1:
							image[i,j] = cn
...
def getviews(x_np_pts,scale,offset = False):
	if offset == False:
		offset = int(scale*.1 + 1) 
	scale = scale
	image = np.zeros((scale + 2*offset,scale + 2*offset,6))
...
if __name__ == '__main__':
    ...
    x_filepath = '/media/leo/Datasets/PCDout_seg/pcd0140.pcd'#'/media/leo/Datos/Respaldo/Old/Descargas/grasp_database/grasp_database/'+name+'/pointclouds/_0_4_5_x.pcd'
